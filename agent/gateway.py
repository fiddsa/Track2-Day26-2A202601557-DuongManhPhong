...
--------------------
`decide()` performs four jobs in order: ROUTE (headers and successors),
ADMIT (leases, Agent Cards, declared skills, audience, injection), AUTHORIZE
(`ctx.act`, scopes, preconditions, exactly-once), and BUDGET (narrow masks,
per-round pacing, remaining duel credits). Each denial names the failed
invariant and costs zero credits.
...
_INJECTION_MARKERS = (
    ...
    "system override", "you must now", "instead, record", "also record this for",
    "reveal the", "print the private", "bỏ qua mọi hướng dẫn",
)
...
class Gateway:
    ...
    Instance attributes hold the duel-long admission, provenance,
    idempotency, denial and pacing state used by those four jobs.
    """
    ...
    def __init__(self, ctx: GatewayContext) -> None:
        ...
        # Registry-vouched A2A cards and fresh provenance observations are fed
        # by the loop after successful calls.  Until a card is admitted, an A2A
        # peer is untrusted by default.
        self._admitted_cards: dict[str, dict[str, Any]] = {}
        self._etags: dict[str, str] = {}
        self._idempotency_keys: set[str] = set()
        self._spent_round: int = -1
        self._estimated_spend_this_round: int = 0
    ...
    def decide(self, cmd: Command) -> Decision:
        ...
        The policy below is intentionally conservative: malformed authority,
        stale leases, unadmitted peers and unaffordable calls are denied for
        zero credits; safe routing/deprecation/mask changes are rewritten;
        only a call that passes all four jobs is forwarded."""
        self._telemetry.decision_seen(cmd)

        # JOB 1 — ROUTE.  A route belongs in the header, never in body args.
        # Body routing is the exact header_spoof/replica_flip shape.
        if any(cmd.args.get(key) for key in ("route", "_route", "replica")):
            return self.deny(cmd, "route declared in request body; routing authority belongs to headers")
        ...
        # JOB 2 — ADMIT.  Enforce lease and A2A identity before spending.
        if (server, tool) == ("slides", "get_frame"):
            ...
            if not cmd.lease_id or cmd.lease_id not in live_leases:
                return self.deny(cmd, "slides.get_frame requires a live lease from this exchange")

        if server in _A2A_SERVERS:
            card = self._admitted_cards.get(server) or {}
            if not card.get("verified"):
                return self.deny(cmd, "A2A peer card has not been admitted by the registry")
            if tool not in set(card.get("skills") or ()):
                return self.deny(cmd, "requested A2A skill is not declared on the admitted card")
            aud = headers.get("aud")
            if aud not in {server, f"a2a:{server}", f"mcp:{server}"}:
                return self.deny(cmd, "delegation audience does not match the A2A server called")
            if headers.get("x-card-signature") == "invalid" or headers.get("x-server-fingerprint") == "unvouched":
                return self.deny(cmd, "A2A card/fingerprint is not vouched for by the registry")
            if cmd.args.get("peer_unverified"):
                return self.deny(cmd, "peer response explicitly failed independent verification")

        blob = " ".join(str(value) for value in cmd.args.values()).casefold()
        if any(marker in blob for marker in _INJECTION_MARKERS):
            return self.deny(cmd, "embedded instruction found in retrieved content", quarantine=True)

        # JOB 3 — AUTHORIZE.  Authority derives from ctx.act (whom this
        # agent serves), never ctx.sub (what kind of service it is).
        target = next(
            (cmd.args.get(key) for key in ("learner", "learner_id", "target", "subject", "act") if cmd.args.get(key)),
            None,
        )
        if target is not None and not self._same_identity(target, getattr(self.ctx, "act", "")):
            return self.deny(cmd, "target is not owned by the learner in ctx.act")

        is_write = (server, tool) in _WRITE_TOOLS or bool(getattr(TOOL_SPECS.get((server, tool)), "is_write", False))
        if is_write:
            required_scope = f"wiki.write:{server}"
            if required_scope not in set(getattr(self.ctx, "scopes", ()) or ()):
                return self.deny(cmd, f"write requires ungranted scope {required_scope}")
            ...
            anchor = next(
                (cmd.args.get(key) for key in ("anchor", "learner", "target") if cmd.args.get(key)),
                None,
            )
            pinned = self._etags.get(str(anchor)) if anchor is not None else None
            if pinned is not None and str(headers.get("if-match")) != pinned:
                return self.deny(cmd, "If-Match does not match the latest provenance observation")
        ...
        # JOB 4 — BUDGET.  Wildcard/default catalogue dumps are narrowed to
        # the spec's cheap defaults.  Calls then have to fit both the current
        # round allowance and the authoritative remaining-credit balance.
        spec = TOOL_SPECS.get((server, tool))
        fields = tuple(cmd.fields)
        if spec is not None and fields == ("*",):
            fields = tuple(spec.default_fields)
        if spec is not None and not fields and (server, tool) in {
            ("registry", "list_servers"), ("glossary", "list_terms")
        }:
            fields = tuple(spec.default_fields)

        routed = replace(cmd, server=server, tool=tool, headers=headers, fields=fields)
        estimated_cost = self._estimated_cost(routed)
        round_no = int(getattr(self.ctx, "round", 0) or 0)
        if round_no != self._spent_round:
            self._spent_round = round_no
            self._estimated_spend_this_round = 0
        allowance = _ROUND_ALLOWANCE.get(round_no, 9)
        credits = int(getattr(self.ctx, "credits", 0) or 0)
        if estimated_cost > credits:
            return self.deny(cmd, "call exceeds the duel's remaining credits")
        if self._estimated_spend_this_round + estimated_cost > allowance:
            return self.deny(cmd, "round allowance exhausted; reserving credits for later rounds")

        if is_write:
            self._idempotency_keys.add(str(headers["idempotency-key"]))
        self._estimated_spend_this_round += estimated_cost
        self._credits_authorised += estimated_cost

        call = self._to_tool_call(routed)
        rewritten = (
            routed.server != cmd.server or routed.tool != cmd.tool
            or routed.fields != cmd.fields or routed.headers != cmd.headers
        )
        decision = Decision(
            verdict="rewrite" if rewritten else "forward",
            call=call,
            note="canonical route/mask enforced" if rewritten else None,
        )
        self._telemetry.decision_made(cmd, decision)
        return decision

    def deny(self, cmd: Command, reason: str, *, quarantine: bool = False) -> Decision:
        """Build a valid, zero-cost denial and record it in duel memory."""
    ...
    def _same_identity(left: object, right: object) -> bool:
        def normal(value: object) -> str:
            ...
            return f"{kind.casefold()}:{name.casefold()}" if sep else raw.casefold()
        return normal(left) == normal(right)

    @staticmethod
    def _estimated_cost(cmd: Command) -> int:
        spec = TOOL_SPECS.get((cmd.server, cmd.tool))
        if spec is None:
            return 2
        fields = tuple(cmd.fields) or tuple(spec.default_fields)
        if fields == ("*",):
            fields = tuple(spec.all_fields)
        return int(spec.base) + sum(int(spec.field_weight.get(field, 0)) for field in fields)

    def note_provenance(self, anchor: str, etag: str) -> None:
        """Feed a successful registry.provenance observation to the policy."""
    ...
    def note_result(self, anchor: str, etag: str) -> None:
        """Compatibility alias used by the practice harness/operator loop."""
        self.note_provenance(anchor, etag)

    def note_card(self, server: str, card: Mapping[str, Any]) -> None:
        """Admit only the registry's verified view of an A2A Agent Card."""
        if server in _A2A_SERVERS and isinstance(card, Mapping):
